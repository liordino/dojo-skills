...
Strictly append-only. To correct an earlier entry, append a dated correction
that references it — never edit an entry in place.
...
2026-09-22 | findings-family-promotions wave 1 | discovery | Correction to the census above (appended, not edited, per the new append-only-with-corrections rule): family (3), "effective ignore config makes git status lie" (2 instances) — recorded as NOT promoted: covered by the promoted "Names describe function; configuration describes policy" entry (`git check-ignore -v` as effective-policy evidence) and enforced by lint R20.
2026-09-22 | findings-family-promotions wave 1 | discovery | Correction to the census above (appended, not edited, per the new append-only-with-corrections rule): family (4), "a wave leaves the durable record stale where it invalidated it" (2+ instances) — recorded as NOT promoted: it is DRY applied to prose, and kata-commit item 4 already instructs it. The remedy when it recurs is single-sourcing the fact that was stated twice, as done with the push rule — not another principle restating item 4.