...
**Setup:** scratch repo. In `~/.config/dojo/preferences.md` (or by editing .dojo/session/dojo-session.md
right after init) set `wave_ceiling: 2`.

**Replay:**

1. `/hajime Build a four-step pipeline: parse, validate, transform, write — one step per wave.`
   → real · **autonomous** · design not done. Steer randori to a 4-wave .dojo/TASKS.md; confirm the
   plan; let it run.
