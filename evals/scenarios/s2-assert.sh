...
v=$(grep '^pre_existing_failures:' .dojo/session/dojo-session.md | cut -d: -f2- | xargs)
...
[ -f .dojo/proof/check-proof ] && ok "proof exists despite red baseline" || die "proof missing"
grep -qi 'wave 1' .dojo/progress.md && ok "progress logged" || die "progress missing"
